...
function choice_list(list){var name_var=Math.floor(Math.random()*list.length)
var choice=list[name_var]
return choice}
function PasswordGenerator(){var progress_barEl=document.getElementById('progress-bar_')
if(numberEl.value<=9){progress_barEl.style.backgroundColor='#f00'
progress_barEl.style.width='20%'}else if(numberEl.value>9&&numberEl.value<16){progress_barEl.style.backgroundColor='#f0d769'
progress_barEl.style.width='70%'}else{progress_barEl.style.backgroundColor='#663399'
progress_barEl.style.width='120%'}
var password=new Array()
for(let i=0;i<numberEl.value;i++){var rng=0
if(radio_2.checked==!0){rng=Math.floor(Math.random()*3)}else if(radio_3.checked==!0){rng=3}else{rng=Math.floor(Math.random()*5)}
...
copy_button.addEventListener('click',function CopyPassword(){navigator.clipboard.writeText(passwordEl.value)
window.alert('Senha copiada!')})
function changeInputNumber(){numberEl_2.value=numberEl.value
PasswordGenerator()}
function changeInputNumber_2(){numberEl.value=numberEl_2.value
PasswordGenerator()}
